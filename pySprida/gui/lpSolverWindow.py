...
class LPSolverWindow(QMainWindow):
    ...
    def __init__(self, container: DataContainer, *args, **kwargs):
        ...
        self.ui.max_different_groups_per_groupType.itemChanged.connect(
            self.update_max_groups_per_groupType)
        ...
        if item.row() == 0:
            ...
            if value.isnumeric():
                if int(value) >= 1 and int(
                    ...
                    self.container.solver_config["lp"]["max_groups_per_groupType"][group_type] = int(
                        value)
                else:
                    info_ok_box(
                        'Use only integers and values between one and the number of groups in this type')

    def set_max_groups_per_groupType_table(self):
        group_type_names = self.container.group_type_names
        max_different = self.container.solver_config["lp"].get(
            "max_groups_per_groupType", {})
        groups_of_type = self.container.get_num_groups_per_type()
        self.ui.max_different_groups_per_groupType.setRowCount(1)
        self.ui.max_different_groups_per_groupType.setColumnCount(
            len(group_type_names))
        for n, gtype in enumerate(group_type_names):
            newitem = QTableWidgetItem(
                str(max_different.get(gtype, groups_of_type[gtype])))
        ...
        self.ui.max_different_groups_per_groupType.setHorizontalHeaderLabels(
            group_type_names)
